...
_DIR="$PREFIX/share/TERMUX-MASTER/colours"
FONTS_DIR="$PREFIX/share/TERMUX-MASTER/fonts"
...
total_colours=$(check_files colours)
...
apply_colors () {
    ...
    # Read user selection
    { echo; read -p ${BLUE}"    [${RED}Select Color Scheme (1 to $total_)${BLUE}]: ${GREEN}" answer; echo; }

    # Apply color-scheme
    if [[ (-n "$answer") && ("$answer" -le $total_) ]]; then
        scheme=${color_schemes[(( answer - 1 ))]}
        echo "    ${BLUE}[${RED}*${BLUE}] ${ORANGE}Applying Color Scheme..."
        cp $_DIR/$scheme $TERMUX_DIR/.properties
...
random_style () {
    ...
    random_scheme=$(ls $_DIR | shuf -n 1)
    { cp $_DIR/$random_scheme $TERMUX_DIR/.properties; }
    if [[ ! -f "$TERMUX_DIR/.current_color" ]]; then
		touch "$TERMUX_DIR/.current_color"
...
import_files () {
    ...
    if [[ "$option" =~ ^[l/L/i/I]$ ]]; then
        if [[ "$option" =~ ^[l/L]$ ]]; then
            ...
            cp $file_path $TERMUX_DIR/.properties
            { reload_settings; reset_color; exit; }
            ...
            cp $temp_file $TERMUX_DIR/.properties
            { reload_settings; reset_color; exit; }
        fi
    else
        echo -n "    ${BLUE}[${RED}!${BLUE}] ${RED}Invalid Option, Try Again."
...
about_program () {
    ...
    │───────────────────────────────────────────────────────────────│
    │                                                               │
    │ >>  'TERMUX-MASTER' is a simple program that provides color    │
    │                                                               │
    │                                                               │
    │                                                               │
    │                                                               │
    │ >>  Author   :  EVILXPLOIT                                    │
    │     Github   :  https://github.com/Evilxploit                   │
    │                                                               │
    └───────────────────────────────────────────────────────────────┘"
...
until [[ "$REPLY" =~ ^[q/Q]$ ]]; do
    ...
    banner
    echo "
    ${BLUE}[${RED}C${BLUE}] ${GREEN} ($total_) (${CYAN}${active_color%.*}${GREEN})
